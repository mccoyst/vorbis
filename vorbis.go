...
func Decode(b []byte) ([]int16, int, error) {
	if len(b) == 0 {
		return nil, 0, nil
	}
	...
	var channels C.int
	var output *C.short
	var samples = C.stb_vorbis_decode_memory(raw, C.int(len(b)), &channels, &output)
	if samples < 0 {
		return nil, 0, errors.New("Failed to decode vorbis")
	}
